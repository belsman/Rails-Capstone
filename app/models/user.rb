class User < ApplicationRecord
  ...
  def who_to_follow
    Following.where("follower_id != #{id}").map do |f|
      f.followed if f.followed_id != id
    end.slice(0, 5).compact.uniq
  end

  def users_who_followed
    followers.map { |f| f.follower if f.follower_id != id }.slice(0, 5).compact
  end
  ...
  def timeline_buzzs
    query_string = 'INNER JOIN followings ON followings.followed_id = buzzs.author_id ' \
                      "AND followings.follower_id = #{id}"
